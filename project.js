...
// ===== 預算（每個專案一筆：預估款 + 工程款 + 已付 + 備註；未付 = 工程款 - 已付）=====

// 讀取某專案的預算。找不到時回 null（前端會顯示空白並可建立）。
...
// 寫入預算。若該專案已有一筆則更新，否則建立。
export async function saveBudget(projectId, values) {
  const { estimate, billed, paid, notes } = values;
  const toNum = v => (v === '' || v === null || v === undefined) ? 0 : Number(v);
  const payload = {
    estimate: toNum(estimate),
    billed: toNum(billed),
    paid: toNum(paid),
    notes: String(notes || ''),
  ...
  // 先查有沒有
  const existing = await getBudget(projectId);
  if (existing) {
    const { data, error } = await supabase
      .from('budget')
      .update(payload)
